...
def main(page: ft.Page):
    ...
    # --- 2. SCIENTIFIC TAB LOGIC (Now with cmath for Complex Numbers) ---
    memory = 0
    history = []
    angle_mode = "DEG"
    shift_active = False
    ...
    # Blending cmath (for complex) and math (for formatting)
    ...
    def safe_sqrt(x): return cmath.sqrt(x) # Automatically handles sqrt(-1) -> 1j
    ...
    def button_clicked(e):
        ...
        # Equals (The Engine)
            ...
            try:
                ...
                # 2. Factorial replacement
                expr = re.sub(r'(\d+\.?\d*)!', r'factorial(\1)', expr)
                
                # --- NEW: IMPLICIT MULTIPLICATION FIXES ---
                expr = re.sub(r'(\d)\(', r'\1*(', expr)  # Fixes 5(2) -> 5*(2)
                expr = re.sub(r'\)(\d)', r')*\1', expr)  # Fixes (2)5 -> (2)*5
                expr = re.sub(r'\)\(', r')*(', expr)     # Fixes (2)(3) -> (2)*(3)
                # ------------------------------------------
                
                # Evaluate the full string using our custom math environment
    ...
    # --- 3. MATRIX / STATE-SPACE TAB (Zero-Lag Pure Python) ---
    ...
    def calculate_matrix(e):
        try:
            a, b = float(m11.value), float(m12.value)
            c, d = float(m21.value), float(m22.value)
            
            det = (a * d) - (b * c)
            trace = a + d
            
            # Eigenvalues using quadratic formula: λ² - Tr(A)λ + Det(A) = 0
    ...
    matrix_tab = ft.Container(
        content=ft.Column([
            ...
            ft.ElevatedButton("Calculate Det & Eigenvalues", on_click=calculate_matrix, bgcolor=ft.colors.BLUE_700, color=ft.colors.WHITE),
            ft.Divider(),
            matrix_output
    ...
    # --- 4. LIVE GRAPHING TAB (Native Flet, No Matplotlib Lag) ---
    ...
    def plot_graph(e):
        try:
            ...
            points = []
            # Calculate 50 points from x = -10 to 10
